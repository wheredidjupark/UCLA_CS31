...
int main()
{
    ...
    user_bill_minutes_over = static_cast<double>(user_minutes_over) *0.40;
    ...
    if(user_text > user_text_BASE)
    {
        ...
        if((user_text_over<=250)&&(user_text_over>0))
        {
            ...
            {
                user_bill_text_over = user_text_over * 0.02;
            }
            else //June thru September
            {
                user_bill_text_over = user_text_over * 0.01;
            }
        }
        else if (user_text_over > 250) //if over 500 messages
        {
            if(((user_month >=1)&&(user_month <= 5))||((user_month >=10)&&(user_month <=12))) //October thru May
            {
                user_bill_text_over = 250 * 0.02;
            }
            else //June thru September
            {
                user_bill_text_over = 250 * 0.01;
            }
            
            user_bill_text_over = user_bill_text_over + (static_cast<double>(user_text-500)*0.11);
        }
    }
